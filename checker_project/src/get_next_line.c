...
/*   Updated: 2022/10/26 20:30:53 by trubat-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_bufconnect(char *buffer, char *temp, int size)
{
	...
	buffer_size = 0 + (!(!buffer)) * ft_strlens(buffer);
...
char	*get_next_line(int fd)
{
	...
	while ((ft_strchr_gnl(buffer, '\n') == NULL) && read_desc != 0)
	{
		...
		buffer = ft_bufconnect(buffer, temp_read, BUFFER_SIZE);
	}
	free(temp_read);
